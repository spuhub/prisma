...
from qgis.core import  QgsVectorFileWriter, QgsVectorLayer, QgsVectorDataProvider, QgsField, QgsFeature
...
class LayoutManager():
    ...
    def identifica_info_sobrep(self):
        ...
        lyr_input = self.dic_layers['input_buffer'] if self.possui_buffer else self.dic_layers['input']
        ...
        for layer_item in self.dic_layers:
            if layer_item not in ('input', 'input_buffer', 'lyr_overlap'):
                ...
                for idx, layer in enumerate(list_type):
                    for feature in lyr_input.getFeatures():
